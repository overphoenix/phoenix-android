...
public class IpfsRealTest {
    ...
    @Test
    public void test_blog_ipfs_io() throws IOException {
        ...
        node = ipfs.resolveNode(link.concat("/").concat(IPFS.INDEX_HTML),
    ...
    public void test_mike() {
        ...
        ipfs.findProviders((peer) -> peers.add(peer.getPeerId()), Cid.decode(res.getHash()),
                true, new TimeoutCloseable(30));
        ...
        for (PeerId peerId : peers) {
            LogUtils.debug(TAG, "connect " + peerId.toBase58() + " " +
                    ipfs.connect(peerId, IPFS.CONNECT_TIMEOUT, IPFS.GRACE_PERIOD,
