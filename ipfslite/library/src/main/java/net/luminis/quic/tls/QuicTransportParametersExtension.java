    ...
    void parseTransportParameter(ByteBuffer buffer, Role senderRol, Logger log) throws DecodeErrorException, InvalidIntegerEncodingException {
        ...
        if (parameterId == original_destination_connection_id.value) {
            ...
            if (StringUtils.isBlank(extension)) {
                msg = String.format(Locale.US,"- unknown transport parameter 0x%04x, size %d", parameterId, size);
            } else {
                msg = String.format(Locale.US,"- unsupported transport parameter 0x%04x, size %d (%s)", parameterId, size, extension);
            }
            log.warn(msg);
