...
import tech.nagual.phoenix.tools.camera.BlurBitmap
import tech.nagual.phoenix.tools.camera.CamConfig
import tech.nagual.phoenix.tools.camera.capturer.ImageCapturer
import tech.nagual.phoenix.tools.camera.capturer.VideoCapturer
import tech.nagual.phoenix.tools.camera.notifier.SensorOrientationChangeNotifier
import tech.nagual.phoenix.tools.camera.ui.*
import tech.nagual.phoenix.tools.camera.ui.seekbar.ExposureBar
import tech.nagual.phoenix.tools.camera.ui.seekbar.ZoomBar
import me.zhanghai.android.files.util.createIntent
import tech.nagual.phoenix.tools.camera.CameraManager
import java.io.File
...
open class CameraActivity : AppCompatActivity(),
    ...
    private var cameraPermissionDialog: androidx.appcompat.app.AlertDialog? = null
    private var audioPermissionDialog: androidx.appcompat.app.AlertDialog? = null
    ...
    private val requestPermissionLauncher = registerForActivityResult(
        ...
        if (permissions.containsKey(Manifest.permission.RECORD_AUDIO)) {
            if (ActivityCompat.checkSelfPermission(this, Manifest.permission.RECORD_AUDIO)
                ...
                builder.setTitle(R.string.camera_audio_permission_dialog_title)
                builder.setMessage(R.string.camera_audio_permission_dialog_message)
...
//        startActivitySafe(GalleryStarterActivity::class.createIntent())
    }
            ...
            if (camConfig.isVideoMode) {
                ...
                captureButton.setImageResource(
                    if (camConfig.isTorchOn) {
                        R.drawable.camera_torch_on_button
                    } else {
                        R.drawable.camera_torch_off_button
                    }
                )
            } else {
                if (timerDuration == 0) {
...
//        })
    ...
    fun onScanResultSuccess(rawText: String) {
        ...
        runOnUiThread {
            ...
            dialog.setContentView(R.layout.camera_scan_result_dialog)
...
//                fTHandler.postDelayed(fTRunnable, focusTimeout * 1000)
    ...
    fun indicateLocationProvidedIsDisabled() {
        val snackbar = Snackbar.make(
            previewView,
            R.string.camera_location_is_disabled,
            Snackbar.LENGTH_SHORT
        )
        snackbar.setAction(R.string.camera_enable) {
    ...
    fun onDeviceAngleChange(xAngle: Float, zAngle: Float) {
        ...
        if (!(camConfig.isVideoMode || camConfig.isVideoMode || cdTimer.isRunning)) {
            ...
            if (gCircle.rotation != xAngle) {
                ...
                gAngleTextView.text = getString(R.string.camera_degree_format, abs(xAngle).toInt())
                if (xAngle == 0f) {
            ...
            if (zAngle.toInt() == 0) {
                gLineX.setBackgroundResource(R.drawable.camera_yellow_shadow_rect)
                gLineZ.visibility = View.GONE

                gLeftDash.setBackgroundResource(R.drawable.camera_yellow_shadow_rect)
                gRightDash.setBackgroundResource(R.drawable.camera_yellow_shadow_rect)

                gAngleTextView.setTextColor(ContextCompat.getColor(this, R.color.camera_z_yellow))

            } else {
                gLineX.setBackgroundResource(R.drawable.camera_white_shadow_rect)
                gLineZ.visibility = View.VISIBLE

                gLeftDash.setBackgroundResource(R.drawable.camera_white_shadow_rect)
                gRightDash.setBackgroundResource(R.drawable.camera_white_shadow_rect)

                gAngleTextView.setTextColor(ContextCompat.getColor(this, R.color.camera_white))
            }
    ...
    fun locationCamConfigChanged(required: Boolean) {
        if (required) {
            ...
            CameraManager.getInstance().dropLocationUpdates()
        }
    }
    ...
    private val enableLocationLauncher = registerForActivityResult(
        ...
        requestLocation(CameraManager.getInstance().isAnyLocationProvideActive())
    }
    ...
    private val locationPermissionLauncher = registerForActivityResult(
        ...
        if (!CameraManager.getInstance().shouldAskForLocationPermission()) {
            requestLocation()
        } else {
        ...
        when {
            ActivityCompat.shouldShowRequestPermissionRationale(
                ...
                MaterialAlertDialogBuilder(
                    this,
                    android.R.style.Theme_DeviceDefault_Dialog_Alert
                ).setTitle(R.string.camera_location_permission_dialog_title)
                    .setMessage(R.string.camera_location_permission_dialog_message)
            ...
            (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION)
                ...
                Manifest.permission.ACCESS_COARSE_LOCATION
                    ...
                    == PackageManager.PERMISSION_GRANTED) -> {
                CameraManager.getInstance().requestLocationUpdates(reAttach)
            }
            else -> {
