package tech.nagual.phoenix.tools.camera.capturer
...
import tech.nagual.phoenix.tools.camera.ui.activities.SecureCameraActivity
import tech.nagual.phoenix.tools.camera.ui.activities.VideoCaptureActivity
...
class VideoCapturer(private val mActivity: CameraActivity) {
        ...
        set(value) {
            if (isRecording) {
                if (value) {
                    recording?.pause()
                    pauseTimer()
                    mActivity.flipCamIcon.setImageResource(R.drawable.camera_play)
                } else {
                    recording?.resume()
                    startTimer()
                    mActivity.flipCamIcon.setImageResource(R.drawable.camera_pause)
                }
            }
            field = value
        }
    ...
    private fun beforeRecordingStarts() {
        ...
        mActivity.timerView.setText(R.string.camera_start_value_timer)
    ...
    private fun afterRecordingStops() {
        ...
        if (mActivity !is VideoCaptureActivity) {
            mActivity.thirdCircle.setImageResource(R.drawable.camera_option_circle)
    ...
    companion object {
        fun isVideo(uri: Uri): Boolean {
            return uri.encodedPath?.contains("video") == true ||
                    uri.encodedPath?.contains("vid") == true ||
