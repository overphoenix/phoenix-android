package tech.nagual.phoenix.tools.organizer.recorder
...
import tech.nagual.phoenix.databinding.DialogRecordAudioBinding
...
class RecordAudioDialog() : BaseDialog<DialogRecordAudioBinding>() {
    ...
    private val connection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            lifecycleScope.launch {
                mediaStorageManager.createMediaFile(MediaStorageManager.MediaType.AUDIO)
                    ?.let { (uri, file) ->
    ...
    override fun createBinding(inflater: LayoutInflater) =
        DialogRecordAudioBinding.inflate(inflater)
    ...
    private fun startRecording() {
        if (recorderService?.isRecording == true) {
            binding.buttonRecord.setImageResource(R.drawable.stop_icon_24dp)
