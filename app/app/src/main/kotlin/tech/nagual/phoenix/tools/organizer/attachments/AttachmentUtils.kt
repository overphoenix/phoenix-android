package tech.nagual.phoenix.tools.organizer.attachments
...
fun Attachment.Companion.fromUri(context: Context, uri: Uri): Attachment {
    ...
    val type = if (mimeType == null) Attachment.Type.GENERIC else when {
        mimeType.startsWith("image") -> Attachment.Type.PHOTO
...
fun getAttachmentUri(context: Context, path: String, mediaFolder: String = OrganizersManager.MEDIA_FOLDER): Uri? {
    return when {
