...
class PercentExpression {
    ...
    PercentExpression(String expression) {
        ...
        if (!matcher.matches() || matcher.group(6) == null ||
                Objects.requireNonNull(matcher.group(6)).length() != 1) {
            // may never getData here but let's be careful
            this.justValue = true;
        ...
        this.totalWidth = readPrecision(matcher.group(2));
        int dotPrecision = readPrecision(matcher.group(4));
        // 5 is ignored
        this.patternChar = Objects.requireNonNull(matcher.group(6)).charAt(0);
    ...
    private static int readPrecision(String string) {
