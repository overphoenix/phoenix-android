package tech.nagual.phoenix.tools.organizer.camera
...
import tech.nagual.phoenix.tools.organizer.camera.ui.activities.CaptureActivity
import tech.nagual.phoenix.tools.organizer.camera.ui.activities.MultiCaptureActivity
import tech.nagual.phoenix.tools.organizer.camera.ui.activities.VideoCaptureActivity
...
class CamConfig(private val mActivity: CameraActivity) {
    ...
    private object CameraModes {
        const val CAMERA = R.string.camera_camera
        const val VIDEO = R.string.camera_video
        const val PORTRAIT = R.string.camera_portrait_mode
        const val HDR = R.string.camera_hdr_mode
        const val NIGHT_SIGHT = R.string.camera_night_sight_mode
        const val FACE_RETOUCH = R.string.camera_face_retouch_mode
        const val AUTO = R.string.camera_auto_mode
    }

    private object SettingValues {

        object Key {
            const val SELF_ILLUMINATION = "organizer_self_illumination"
            const val GEO_TAGGING = "organizer_geo_tagging"
            const val FLASH_MODE = "organizer_flash_mode"
            const val GRID = "organizer_grid"
            const val EMPHASIS_ON_QUALITY = "organizer_emphasis_on_quality"
            const val FOCUS_TIMEOUT = "organizer_focus_timeout"
            const val VIDEO_QUALITY = "organizer_video_quality"
            const val ASPECT_RATIO = "organizer_aspect_ratio"
            const val INCLUDE_AUDIO = "organizer_include_audio"

            const val SAVE_IMAGE_AS_PREVIEW = "organizer_save_image_as_preview"

            const val STORAGE_LOCATION = "organizer_storage_location"
            const val MEDIA_URIS = "organizer_media_uri_s"

            const val PHOTO_QUALITY = "organizer_photo_quality"

            const val REMOVE_EXIF_AFTER_CAPTURE = "organizer_remove_exif_after_capture"

            const val GYROSCOPE_SUGGESTIONS = "organizer_gyroscope_suggestions"

            // const val IMAGE_FILE_FORMAT = "organizer_image_quality"
            // const val VIDEO_FILE_FORMAT = "organizer_video_quality"
        }
    ...
    var focusTimeout = 5L
        set(value) {
            val option = if (value == 0L) {
                mActivity.getString(R.string.camera_focus_timeout_off)
            } else {
                "${value}s"
            }
    ...
    val mediaUris: ArrayList<Uri>
        get() {
            ...
            val uriPaths = if (data.isEmpty()) {
                ...
                data.split(CamConfig.PATH_SEPARATOR)
            }
    ...
    val isInCaptureMode: Boolean
        get() {
            return mActivity is CaptureActivity || mActivity is MultiCaptureActivity
        }
...
//        if (mActivity !is SecureCameraActivity) {
...
//        }
    ...
    fun switchMode(modeText: Int) {
        ...
        mActivity.flipCamIcon.setImageResource(R.drawable.camera_flip_camera)
        mActivity.cancelButtonView.visibility = View.VISIBLE
        mActivity.previewView.scaleType = PreviewView.ScaleType.FIT_START

        mActivity.captureButton.setBackgroundResource(R.drawable.camera_cbutton_bg)

        if (isVideoMode) {
            mActivity.captureButton.setImageResource(R.drawable.camera_recording)
        } else {
