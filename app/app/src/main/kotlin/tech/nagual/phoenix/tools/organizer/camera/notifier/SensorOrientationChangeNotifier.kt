package tech.nagual.phoenix.tools.organizer.camera.notifier
...
class SensorOrientationChangeNotifier private constructor(
    ...
    companion object {
        ...
        fun getInstance(mActivity: CameraActivity): SensorOrientationChangeNotifier? {
    ...
    var mOrientation = cameraActivity.getRotation()
        private set
    ...
    inner class NotifierSensorEventListener : SensorEventListener {
        ...
        override fun onSensorChanged(event: SensorEvent) {
            ...
            val dAngle = if (cameraActivity.gCircleFrame.rotation == 270f) {
                90f
            } else {
                cameraActivity.gCircleFrame.rotation
            }

            val zAngle = (atan(z) / (Math.PI / 180)).toFloat()

            if (zAngle < Z_EXIT_MIN || zAngle > Z_EXIT_MAX) {
                cameraActivity.gCircleFrame.visibility = View.GONE
                return
            }
                ...
                if (abs(fAngle - lastX) < X_THRESHOLD) {
                    if (zAngle in Z_ENTRY_MIN..Z_ENTRY_MAX) {
                        if (abs(zAngle - lastZ) < Z_THRESHOLD) {
                            cameraActivity.gCircleFrame.visibility = View.VISIBLE
                        }
                    }
                }
            }
        ...
        fun updateGyro(fAngle : Float, zAngle : Float) {
            cameraActivity.onDeviceAngleChange(fAngle, zAngle)
    ...
    fun notifyListeners(manualUpdate : Boolean = false) {
        ...
        if(manualUpdate){
            mOrientation = cameraActivity.getRotation()
        }
    ...
    init {
        ...
        mSensorManager = cameraActivity.getSystemService(Context.SENSOR_SERVICE) as SensorManager
    }
}