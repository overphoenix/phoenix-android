...
CUSTOM_THEME_COLOR_KT='../app/src/main/java/tech/nagual/phoenix/theme/custom/ThemeColor.kt'
...
for color_index in "${!CUSTOM_COLORS[@]}"; do
    for theme_suffix in "${THEME_SUFFIXES[@]}"; do
        ...
        cat >>"${CUSTOM_THEMES_XML}" <<EOF
    <style name="Theme.Phoenix${theme_suffix}.${color}">
...
for color_index in "${!CUSTOM_COLORS[@]}"; do
    for theme_suffix in "${THEME_SUFFIXES[@]}"; do
        color="${CUSTOM_COLORS[color_index]}"
        cat >>"${CUSTOM_THEMES_XML}" <<EOF
    <style name="Theme.Phoenix${theme_suffix}.${color}.Black" />
EOF
    done
done
cat >>"${CUSTOM_THEMES_XML}" <<EOF
...
package tech.nagual.phoenix.theme.custom
