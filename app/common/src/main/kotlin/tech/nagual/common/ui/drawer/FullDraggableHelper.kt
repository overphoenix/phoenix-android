package tech.nagual.common.ui.drawer
...
class FullDraggableHelper(private val context: Context, private val callback: Callback) {
    ...
    private val swipeSlop: Int
    private val distanceThreshold: Int
    private val xVelocityThreshold: Int
    ...
    fun onInterceptTouchEvent(event: MotionEvent): Boolean {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (canNestedViewScroll(
                    callback.drawerMainContainer,
                    false,
                    (x - lastMotionX).toInt(),
                    x.toInt(),
                    y.toInt()
            ...
            intercepted =
                Math.abs(diffX) > touchSlop && Math.abs(diffX) > Math.abs(y - initialMotionY) && isDrawerEnabled(
                    diffX
                )
        }
        return intercepted
    ...
    private val isDrawerOpen: Boolean
        private get() = callback.isDrawerOpen(Gravity.LEFT) || callback.isDrawerOpen(Gravity.RIGHT)
