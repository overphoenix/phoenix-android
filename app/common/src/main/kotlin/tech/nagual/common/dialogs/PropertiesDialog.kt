...
import tech.nagual.common.databinding.DialogPropertiesBinding
...
class PropertiesDialog() {
    ...
    constructor(activity: Activity, path: String, countHiddenItems: Boolean = false) : this() {
        ...
        ensureBackgroundThread {
            ...
            activity.runOnUiThread {
                (view.findViewById<LinearLayout>(R.id.properties_size)
                    .findViewById<TextView>(R.id.property_value)).text =
                    size

                if (fileDirItem.isDirectory) {
                    (view.findViewById<LinearLayout>(R.id.properties_file_count)
                        .findViewById<TextView>(R.id.property_value)).text =
                        fileCount.toString()
                    (view.findViewById<LinearLayout>(R.id.properties_direct_children_count)
                        .findViewById<TextView>(R.id.property_value)).text =
            ...
            if (!fileDirItem.isDirectory) {
                ...
                val exif = if (activity.isPathOnOTG(fileDirItem.path)) {
                    ExifInterface(
                        (activity as tech.nagual.common.activities.BaseSimpleActivity).getFileInputStreamSync(
                            fileDirItem.path
                        )!!
                    )
        ...
        if (fileDirItem.isDirectory) {
            addProperty(
                ...
                fileDirItem.getLastModified(activity).formatDate(activity)
            )
        } else {
    ...
    private fun addExifProperties(path: String, activity: Activity) {
        val exif = if (activity.isPathOnOTG(path)) {
            ExifInterface((activity as tech.nagual.common.activities.BaseSimpleActivity).getFileInputStreamSync(path)!!)
...
//            property_label.setTextColor(mActivity.baseConfig.textColor)
            ...
            findViewById<TextView>(R.id.property_label).text = mResources.getString(labelId)
            findViewById<TextView>(R.id.property_value).text = value
            binding.propertiesHolder.addView(this)

            setOnLongClickListener {
                mActivity.copyToClipboard(findViewById<TextView>(R.id.property_value).value)
                true
            }
