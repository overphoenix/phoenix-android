...
import tech.nagual.common.preferences.preferences.*
import tech.nagual.common.preferences.preferences.choice.MultiChoiceDialogPreference
import tech.nagual.common.preferences.preferences.choice.SelectionItem
import tech.nagual.common.preferences.preferences.choice.SingleChoiceDialogPreference
...
inline fun PreferenceScreen.Appendable.themeColor(
    key: String,
    block: ThemeColorPreference.() -> Unit) : ThemeColorPreference {
    return ThemeColorPreference(key).apply(block).also(::addPreferenceItem)
}

inline fun PreferenceScreen.Appendable.list(
    key: String,
    block: ListPreference.() -> Unit) : ListPreference {
    return ListPreference(key).apply(block).also(::addPreferenceItem)
}
...
inline fun MultiChoiceDialogPreference.onSelectionChange(crossinline callback: (Set<String>) -> Boolean) {
    selectionChangeListener =
        MultiChoiceDialogPreference.OnSelectionChangeListener { _, selection ->
            callback(selection)
        }
}
